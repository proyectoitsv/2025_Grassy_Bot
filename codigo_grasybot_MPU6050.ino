    ...
    if (c == '\n') {
      ...
      if(modo == 0){
        ...
        if (receivedData.startsWith("AVA")) {
          ...
          Serial.print("Avanzar: "); Serial.println(x);
          avanzar(x);
        }
        if (receivedData.startsWith("REV")) {
          String numeroStr = receivedData.substring(3);
          int x = numeroStr.toInt();
          Serial.print("Retroceder: "); Serial.println(x);
          retroceder(x);
        }
        if (receivedData.startsWith("DER")) {
          String numeroStr = receivedData.substring(3);
          int x = numeroStr.toInt();
          Serial.print("Derecha: "); Serial.println(x);
          derecha(x);
        }
        if (receivedData.startsWith("IZQ")) {
          String numeroStr = receivedData.substring(3);
          int x = numeroStr.toInt();
          Serial.print("Izquierda: "); Serial.println(x);
          izquierda(x);
        }
        if (receivedData.startsWith("MOT")) {
          cuchilla = !cuchilla;
          digitalWrite(Motor, cuchilla);
          Serial.print("Motor: "); Serial.println(cuchilla ? "ON" : "OFF");
        }
      ...
      if(modo == 2){
        if (receivedData.startsWith("ANC")) {
          ...
          int x = numeroStr.toInt();
          ancho = x;
          Serial.print("Ancho: "); Serial.println(ancho);
        }
        if (receivedData.startsWith("LAR")) {
          String numeroStr = receivedData.substring(3);
          int x = numeroStr.toInt();
          largo = x;
          Serial.print("Largo: "); Serial.println(largo);
        }
        if (receivedData.startsWith("ESP")) {
          modo = 0;
          Serial.println("Modo ESPERA activado");
